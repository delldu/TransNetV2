...
def create_test_tfrecord(video_fn, scenes_fn, target_fn, width, height, six_channels=False):
    ...
    if six_channels:
        frame_centers = video_utils.get_frames(video_fn, width * 3, height * 3)[
...
def create_train_dataset(target_dir, target_fn, mapping_fn, width, height, n_videos_in_tfrecord=20, six_channels=False):
    ...
    for start_idx in range(0, len(mapping), n_videos_in_tfrecord):
        ...
        for video_fn, scenes_fn in mapping[start_idx : start_idx + n_videos_in_tfrecord]:
            tfrecord_scenes.extend(get_scenes_from_video(video_fn, scenes_fn, width, height, six_channels=six_channels))
            pbar.update()
        ...
        with tf.io.TFRecordWriter(
            os.path.join(target_dir, "{}-{:04d}.tfrecord".format(target_fn, start_idx)), options
        ) as writer:
...
def create_train_transition_dataset(target_dir, target_fn, mapping_fn, width, height, n_videos_in_tfrecord=50):
    ...
    for start_idx in range(0, len(mapping), n_videos_in_tfrecord):
        tfrecord_scenes = []
        for video_fn, scenes_fn in mapping[start_idx : start_idx + n_videos_in_tfrecord]:
            tfrecord_scenes.extend(get_transitions_from_video(video_fn, scenes_fn, width, height))
            pbar.update()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "type",
        type=str,
        choices=["train", "test", "train-transitions", "test-npy"],
        help="type of tfrecord to generate",
    )
    parser.add_argument(
        "--mapping_fn",
        type=str,
        help="path to mapping file containing lines in following format: " "/path/to/video.mp4,/path/to/scenes/gt",
        required=True,
    )
    ...
    if args.type == "train":
        ...
        create_train_dataset(
            args.target_dir, args.target_fn, args.mapping_fn, args.w, args.h, six_channels=args.six_channels
        )
