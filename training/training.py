...
class Trainer:
    def __init__(
        ...
        self.mean_metrics = dict(
            [
                ...
                for name in ["loss/total", "loss/one_hot_loss", "loss/many_hot_loss", "loss/l2_loss", "loss/comb_reg"]
            ]
        )
        self.results = {}
    ...
    def compute_loss(
        ...
        with tf.name_scope("losses"):
            if bi_tempered_loss:
                one_hot_loss = bi_tempered_binary_logistic_loss(
                    activations=one_hot_pred,
                    labels=tf.cast(one_hot_gt, tf.float32),
                    t1=bi_tempered_loss_temp1,
                    t2=bi_tempered_loss_temp2,
                    label_smoothing=0.0,
                )
            else:
                one_hot_loss = tf.nn.sigmoid_cross_entropy_with_logits(
                    logits=one_hot_pred, labels=tf.cast(one_hot_gt, tf.float32)
                )
            ...
            many_hot_loss = 0.0
            if many_hot_loss_weight != 0.0 and many_hot_pred is not None:
                many_hot_loss = many_hot_loss_weight * tf.reduce_mean(
                    tf.nn.sigmoid_cross_entropy_with_logits(
                        logits=many_hot_pred[:, :, 0], labels=tf.cast(many_hot_gt, tf.float32)
                    )
                )

            l2_loss = 0.0
            if l2_loss_weight != 0.0:
                l2_loss = l2_loss_weight * tf.add_n(
                    [tf.nn.l2_loss(v) for v in self.net.trainable_weights], name="l2_loss"
                )

            total_loss = one_hot_loss + many_hot_loss + l2_loss
            losses = {"loss/one_hot_loss": one_hot_loss, "loss/many_hot_loss": many_hot_loss, "loss/l2_loss": l2_loss}
            ...
            if i % self.log_freq == self.log_freq - 1:
                ...
                with self.summary_writer.as_default():
                    visualizations = visualization_utils.visualize_predictions(
                        frame_sequence.numpy()[:, :, :, :, :3],
                        logit_fc(one_hot_pred).numpy(),
                        one_hot_gt.numpy(),
                        logit_fc(many_hot_pred).numpy() if many_hot_pred is not None else None,
                        many_hot_gt.numpy(),
                    )
    ...
    def test_epoch(self, datasets, epoch_no, save_visualization_to=None, trace=False, logit_fc=tf.sigmoid):
        ...
        for ds_name, dataset in datasets:
            ...
            with self.summary_writer.as_default():
                ...
                f1 = metrics_utils.create_scene_based_summaries(
                    one_hot_pred_list, one_hot_gt_list, prefix="test/" + ds_name, step=epoch_no
                )
...
if __name__ == "__main__":
    ...
    tst_ds = [(name, input_processing.test_pipeline(files)) for name, files in options["tst_files"].items()]
    ...
    if options["test_only"]:
        trainer.test_epoch(
            tst_ds, 0, os.path.join(options["log_dir"], "visualization-00"), trace=True, logit_fc=logit_fc
        )
        exit()
    ...
    for epoch in range(1, options["n_epochs"] + 1):
        ...
        trainer.test_epoch(
            tst_ds,
            epoch,
            os.path.join(options["log_dir"], "visualization-{:02d}".format(epoch)),
            trace=epoch == 1,
            logit_fc=logit_fc,
        )
