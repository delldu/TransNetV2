...
def train_pipeline(
    ...
    ds = ds.interleave(
        lambda x: tf.data.TFRecordDataset(x, compression_type="GZIP").map(parse_train_sample, num_parallel_calls=1),
        cycle_length=8,
        block_length=16,
...
@gin.configurable(blacklist=["filenames"])
def train_transition_pipeline(filenames, shuffle_buffer=100, batch_size=16, repeat=False):
    ds = tf.data.Dataset.from_tensor_slices(filenames)
    ds = ds.shuffle(len(filenames))
    ds = ds.interleave(
...
def parse_train_transition_sample(sample, shot_len=None, frame_width=48, frame_height=27):
...
@tf.function
@gin.configurable(blacklist=["sample"])
def parse_train_sample(
    sample,
    shot_len=None,
    frame_width=48,
    frame_height=27,
    sudden_color_change_prob=0.0,
    ...
    features = tf.io.parse_single_example(
        sample, features={"scene": tf.io.FixedLenFeature([], tf.string), "length": tf.io.FixedLenFeature([], tf.int64)}
    )
    length = tf.cast(features["length"], tf.int32)
    ...
    if sudden_color_change_prob != 0.0:
        ...
        def color_change(shot_):
            ...
            start = augment_shot(start, up_down_flip_prob=0.0, left_right_flip_prob=0.0)
            return tf.concat([start, end], axis=0)

        scene = tf.cond(tf.random.uniform([]) < sudden_color_change_prob, lambda: color_change(scene), lambda: scene)
...
def augment_shot_spacial(
    ...
    shot = tf.cond(tf.random.uniform([]) < random_shake_prob, lambda: random_shake(shot), lambda: shot)
...
def augment_shot(
    ...
    shot = tf.cond(tf.random.uniform([]) < up_down_flip_prob, lambda: tf.image.flip_up_down(shot), lambda: shot)

    assert no_channels == 3 or left_right_flip_prob == 0.0
    shot = tf.cond(tf.random.uniform([]) < left_right_flip_prob, lambda: tf.image.flip_left_right(shot), lambda: shot)
    ...
    if adjust_contrast:
        ...
        shot = tf.clip_by_value(shot, 0.0, 1.0)
    ...
    shot = tf.clip_by_value(shot, 0.0, 1.0) * 255.0

    if color_prob != 0.0:
        factor = tf.random.uniform([], minval=color_min_val, maxval=color_max_val)
        shot = tf.cond(tf.random.uniform([]) < color_prob, lambda: pil_color(shot, factor), lambda: shot)

    if equalize_prob != 0.0 or posterize_prob != 0.0:
        shot = tf.cast(shot, tf.uint8)
        shot = tf.cond(tf.random.uniform([]) < equalize_prob, lambda: pil_equalize(shot), lambda: shot)

        bits = tf.random.uniform([], minval=posterize_min_bits, maxval=7, dtype=tf.int32)
        shot = tf.cond(
            tf.random.uniform([]) < posterize_prob,
            lambda: pil_posterize(shot, bits=tf.cast(bits, tf.uint8)),
            lambda: shot,
        )
...
def concat_shots(
    ...
    if color_transfer_prob > 0:
        assert no_channels == 3  # not implemented
        shot2 = tf.cond(
            tf.random.uniform([]) < color_transfer_prob,
            lambda: color_transfer(source=shot1, target=shot2),
            lambda: shot2,
        )
    ...
    # dissolve
    dis_len = tf.random.uniform(
        [], minval=transition_min_len // 2, maxval=(transition_max_len // 2) + 1, dtype=tf.int32
    )
    dis_kernel = tf.linspace(1.0, 0.0, dis_len * 2 + 2)[1:-1]
    ...
    dissolve_trans = tf.reshape(
        tf.cast(tf.logical_and(tf.not_equal(dissolve, 0.0), tf.not_equal(dissolve, 1.0)), tf.int32), [shot_len]
    )

    # switch between hard cut and dissolve
    is_dissolve = tf.random.uniform([]) > hard_cut_prob
    transition, many_hot_gt = tf.cond(is_dissolve, lambda: (dissolve, dissolve_trans), lambda: (hard_cut, one_hot_gt))
    ...
    shot = tf.cond(
        tf.logical_and(is_dissolve, tf.random.uniform([]) < advanced_shot_trans_prob),
    ...
    if cutout_prob > 0.0:
        assert no_channels == 3  # not implemented
        shot = tf.cond(tf.random.uniform([]) < cutout_prob, lambda: cutout(shot), lambda: shot)
...
def advanced_shot_transitions(shot1, shot2, trans_interpolation):
    ...
    shot1, shot2 = tf.cond(
        flip_wh, lambda: (tf.transpose(shot1, [0, 2, 1, 3]), tf.transpose(shot2, [0, 2, 1, 3])), lambda: (shot1, shot2)
    )
    ...
    shot1 = tf.cond(
        tf.cast(tf.random.uniform([], maxval=2, dtype=tf.int32), tf.bool), lambda: shot1_out, lambda: shot1 * shot1_mask
    )
    shot2 = tf.cond(
        tf.cast(tf.random.uniform([], maxval=2, dtype=tf.int32), tf.bool), lambda: shot2_out, lambda: shot2 * shot2_mask
    )
...
def cutout(
    ...
    if cutout_color is not None:
        ...
        t = tf.random.uniform([1, height, width, 3], 0, 255.0, dtype=tf.float32)

    random_patch = tf.pad(t, [[0, 0], [top, frame_height - bottom], [left, frame_width - right], [0, 0]])
    mask = tf.pad(
        tf.zeros([1, height, width, 1]),
        [[0, 0], [top, frame_height - bottom], [left, frame_width - right], [0, 0]],
        constant_values=1.0,
    )
...
def pil_color(shot, factor):
    ...
    return tf.clip_by_value(degenerate + scaled, 0.0, 255.0)
...
def rgb_to_lab(rgb):
    with tf.name_scope("rgb_to_lab"):
        srgb_pixels = tf.reshape(rgb, [-1, 3]) / 255.0
        ...
        with tf.name_scope("xyz_to_cielab"):
            ...
            xyz_normalized_pixels = tf.multiply(
                xyz_pixels, tf.constant([[1 / 0.950456, 1.0, 1 / 1.088754]], tf.float32)
            )
...
def lab_to_rgb(lab):
    with tf.name_scope("lab_to_rgb"):
        ...
        return tf.reshape(rgb_pixels, tf.shape(lab)) * 255.0


@gin.configurable(blacklist=["filenames"])
def test_pipeline(filenames, shot_len=100, batch_size=16):
    ds = tf.data.Dataset.from_tensor_slices(filenames)
    ds = ds.interleave(
        lambda x: tf.data.TFRecordDataset(x, compression_type="GZIP")
        .map(parse_test_sample, num_parallel_calls=1)
        .batch(shot_len, drop_remainder=True),
        cycle_length=8,
        block_length=16,
        num_parallel_calls=tf.data.experimental.AUTOTUNE,
    )
...
@tf.function
@gin.configurable(blacklist=["sample"])
def parse_test_sample(sample, frame_width=48, frame_height=27, no_channels=3):
    features = tf.io.parse_single_example(
        sample,
        features={
